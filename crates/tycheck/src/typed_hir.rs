...
pub enum TypedExpr {
  ...
  For {
    ...
    acc_init: Option<TypedDatabaseIdx>,
    fold_acc: Option<String>,
    fold_index: Option<String>,
    body: TypedDatabaseIdx,
    ty: Ty,
  },
  /// Struct name in scope; construct values with `new Name { }`, not `Name(...)`.
  StructConstructor {
    name: String,
  },
  StructInstance {
...
impl TypedExpr {
  pub fn ty(&self) -> Ty {
    match self {
      ...
      Self::StructInstance { name } => Ty::Instance(name.clone()),
